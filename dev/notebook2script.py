...
def notebook2script(fname=None, allFiles=True, upTo=None):
    ...
    '''
    # initial checks
    if allFiles is None: allFiles=True # Enable allFiles if upTo is present
    if (fname is None) and (not allFiles): print('Should provide a file name')
    if not allFiles: 
        notebook2scriptSingle(fname)
        print(fname)
        if fname == 'TSUtilities.ipynb':
            notebook2scriptSingle(fname, tgt_folder='.')
    else:
        print('Begin...')
...
def notebook2scriptSingle(fname, tgt_folder='exp'):
    "Finds cells starting with `#export` and puts them into a new module"
    fname = Path(fname)
    fname_out = f'nb_{fname.stem.split("_")[0]}.py'
    main_dic = json.load(open(fname,'r'))
    code_cells = [c for c in main_dic['cells'] if is_export(c)]
    module = f'''#AUTOGENERATED! DO NOT EDIT! file to edit: ./{fname.name} (unless otherwise specified)
'''
    for cell in code_cells: module += ''.join(cell['source'][1:]) + '\n\n'
    # remove trailing spaces
    module = re.sub(r' +$', '', module, flags=re.MULTILINE)
    if tgt_folder is not '.': 
        directory = fname.parent/'exp' #directory = '..'/fname.parent/'exp'
    else:
        directory = fname.parent #directory = '..'/fname.parent/'exp'
    output_path = directory/fname_out
